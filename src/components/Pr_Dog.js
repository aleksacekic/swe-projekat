...
function Pr_Dog() {
  ...
  const BlokirajFunc = async (ID) => {
    ...
    await fetch(`https://localhost:7186/Korisnik/BlokirajKorisnika/${ID}`,{method: `PUT`});
  ...
  const ObrisiFunc = async (ID_PR, ID_DOG) => {
      ...
      await fetch(`https://localhost:7186/Razlog/IzbrisiRazlogeDogadjaja/${ID_DOG}`,{method: `DELETE`,});
      await fetch(`https://localhost:7186/Pr_dog/IzbrisiPrijavljeniDogadjaj/${ID_PR}`, {method: `DELETE`,});
      await fetch(`https://localhost:7186/Dogadjaj/IzbrisiDogadjaj/${ID_DOG}`, {method: `DELETE`,});
  ...
  const IgnorisiFunc = async (ID_PR, ID_DOG) => {
    ...
    const fetchDogadjaji = () => {
        const url = `https://localhost:7186/Pr_dog/VratiPrijavljene_dog/${brojPosiljke}/${ukupnoElemenata}`;
        fetch(url)
        .then(res => res.json())
        .then(data => {
    ...
    return (
      <div className='pr_dog_klasa'>
        ...
        <div className="job_descp">
          ...
          <p>{dogadjaj.dogadjaj_Id.opis}<a href="#">view more</a></p>
          {dogadjaj.dogadjaj_Id.dogadjajImage && (
            <img src={`https://localhost:7186/resources/${dogadjaj.dogadjaj_Id.dogadjajImage}`} className='rounded float-left dogadjaj-slika' />
          )}
          <HideShowMapa 
      ...
      <button onClick={() => UcitajDalje()}>Ucitaj jos dogadjaja...</button>
      </div>
       );
